...
class DataCollector:
    ...
    def save_data(self):
        ...
        image_filename = f"exp2/exp_{self.count}.jpg"
        pose_filename = f"exp2/exp_{self.count}.txt"
    ...
    def run(self):
        ...
        num_r_pitch = 12
        num_r_yaw = 10
        camera_pitch = math.pi/2 #我之前一直搞错了，原来pitch是和z轴的夹角，而非和xy平面的夹角
        camera_yaw = 0
        camera_roll = 0
        flag_clockwise = True
        ...
        # 应该用numpy进行并行计算的，结果写了循环，幸好这个任务对速度没有要求
